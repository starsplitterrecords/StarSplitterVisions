import { coverAssets, pageImageBases } from './assets'
import { buildDailyPagesFromReleaseConfig } from '../utils/dailyPages'
import { validateReleaseConfig } from '../utils/releaseValidation'
import type { ReleaseConfig, SeriesPageData, SeriesPagesBySlug } from '../types/series'

const defaultPurchaseLinks = ['Comixology', 'Kindle', 'Apple Books', 'Google Play Books']

const vikingsIssue01ReleaseConfig: ReleaseConfig = {
  seriesSlug: 'vikings-2026',
  releaseSlug: 'issue-01',
  pageCount: 15,
  startDate: '2026-05-01',
  cadence: 'daily',
  imagePathBase: pageImageBases.vikingsIssue01,
}
...
type ComingSoonSeriesInput = Pick<
  SeriesPageData,
  | 'slug'
  | 'title'
  | 'worldLabel'
...
function createComingSoonSeries({
  ...
  accent,
  atmosphere,
}: ComingSoonSeriesInput): SeriesPageData {
  return {
    slug,
    title,
    worldLabel,
    ...
    dailyPages: [],
    releases: [],
    purchaseLinks: defaultPurchaseLinks,
    status: 'coming-soon',
  }
}

export const seriesPages: SeriesPagesBySlug = {
  'vikings-2026': {
    slug: 'vikings-2026',
    title: 'Vikings 2026',
    ...
    hero: coverAssets.vikingsIssue01,
    accent: '#BAFF00',
    ...
    purchaseLinks: defaultPurchaseLinks,
    status: 'active',
    format: 'Daily comic / prestige ensemble satire',
    audiencePromise: 'A grounded social satire where ancient honor systems collide with modern institutional life.',
    worldPremise: 'When temporal displacement events begin depositing Viking-era people into modern New York, the Department of Temporal Integration is forced to normalize the impossible through paperwork, housing, mediation, and civic process.',
    coreConflict: 'Human identity versus institutional assimilation.',
    seriesEngine: 'Modern bureaucracy processing mythic people as recurring municipal reality.',
    narrativeForms: ['Temporal Bureaucracy', 'Workplace Satire', 'Ensemble Drama'],
    themes: ['Institutional adaptation', 'Belonging', 'Public systems', 'Identity under pressure'],
    readerEntry: 'Start with the daily pages. The world reveals itself socially rather than through exposition.',
    developmentStatus: 'Active release',
  },
  'azure-reach': createComingSoonSeries({
    slug: 'azure-reach',
    title: 'Azure Reach',
    worldLabel: 'Luxury Marine Park // Public Experience Division',
    tagline: 'Invisible labor keeps the wonder alive.',
    description: 'A workplace ensemble dramedy about the people maintaining a social-media-famous marine park built on spectacle, logistics, and emotional labor.',
    hero: coverAssets.azureReachIssue01,
    accent: '#49D7FF',
    atmosphere: 'Aquarium glass, backstage corridors, influencer culture, operational stress, and carefully maintained wonder.',
  }),
  'glass-hour': createComingSoonSeries({
    ...
    hero: coverAssets.glassHourIssue01,
    accent: '#FF4FD8',
  ...
  'signal-atlas': createComingSoonSeries({
    ...
    hero: coverAssets.signalAtlasIssue01,
    accent: '#76FFE5',
  ...
  'hollow-creek': createComingSoonSeries({
    ...
    hero: coverAssets.hollowCreekIssue01,
    accent: '#7D8EFF',
  ...
  'the-choir-array': createComingSoonSeries({
    ...
    hero: coverAssets.theChoirArrayIssue01,
    accent: '#D8A6FF',
  ...
  'supersonic-being': createComingSoonSeries({
    ...
    hero: coverAssets.supersonicBeingIssue01,
    accent: '#FF5B5B',
