...
export default function App() {
  ...
  const isExtrasIndex = path === '/extras';
  const extraSlug = path.startsWith('/extras/') ? path.replace('/extras/', '') : null;
  ...
  const extra = data.extras.find((item) => item.slug === extraSlug || item.id === extraSlug);
  const soundtracksBySeries = useMemo(() => (Array.isArray(data.soundtracks) ? data.soundtracks : []).reduce((map, item) => {
    if (!item?.seriesSlug) return map;
    map.set(item.seriesSlug, [...(map.get(item.seriesSlug) || []), item]);
    return map;
  ...
  useEffect(() => {
    if (isExtrasIndex) document.title = 'Extras | Star Splitter Visions';
    else if (extra) document.title = `${extra.title} | Star Splitter Visions`;
    else document.title = 'Star Splitter Visions';
  }, [extra, isExtrasIndex]);
  ...
  if (series && !release && !readReleaseId) pageContent = <SeriesPage series={series} releases={data.releases} extras={data.extras} allSeries={data.series} soundtracksBySeries={soundtracksBySeries} />;
  if (releaseId && release) pageContent = <ReleasePage release={release} series={data.series.find((item) => item.slug === release.seriesSlug) || { slug: '', title: 'Series' }} pages={data.pages} />;
  if (readReleaseId && release) pageContent = <ReaderPage release={release} pages={getReleasedPagesForRelease(data.pages, release.id)} series={data.series.find((item) => item.slug === release.seriesSlug)} soundtracks={data.soundtracks} extras={data.extras} />;
  if (readReleaseId && !release) pageContent = <main className="page page-reader page-reader-empty"><h1>Release not found.</h1><p><a href="/">Return home</a></p></main>;
  if (isExtrasIndex) pageContent = <ExtrasPage extras={data.extras} series={data.series} />;
  if (extra) pageContent = <ExtraDetailPage extra={extra} series={data.series.find((item) => item.slug === extra.seriesSlug)} release={data.releases.find((item) => item.id === extra.releaseSlug)} />;

  return <><SiteHeader />{pageContent}</>;
}
